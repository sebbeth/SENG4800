...
void AGlowCube::applyState(float delta) {
	/*
	...
	*/
	...
	FVector target = (startPoint + directionVector * (totalYDistance*delta));
	...
	//double strafeSinValue = std::sin(delta*(M_PI*STRAFE_SIN_REPETITIONS));
	//
	...
	////now do scaling
	////the value of the sin function at the current point in time
	//double scaleSinValue = std::sin(delta*(M_PI*SCALE_SIN_REPETITIONS));
